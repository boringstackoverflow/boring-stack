...
### Added
- `boringstack` standard-library Go CLI with `new`, `init`, `dev`, `doctor`, `deploy`, `version`, and dry-run support.
- Canonical runnable project scaffold with embedded templates/assets, graceful shutdown, `/healthz`, generated smoke tests, stack decision record, agent guidance, and production deploy files.
- Safety and integration coverage for path validation, overwrite refusal, generated-project test/vet/build, and a live HTTP smoke test.

### Changed
- The one-line installer now builds the CLI into `~/.local/bin` when Go 1.22+ is present while preserving skill-only installation when it is not.
- The skill now invokes the canonical CLI scaffold before adding a prompt-specific product slice instead of carrying a duplicate `main.go` recipe.
- Corrected stack intake copy from four questions to five.
...
- 5-question stack-picker intake that emits `STACK.md` + a `CLAUDE.md` anchor so the chosen stack survives across sessions and resists drift.
